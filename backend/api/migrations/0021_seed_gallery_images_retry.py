...
def seed_gallery_images_retry(apps, schema_editor):
    ...
    for filename, title, description, order in items:
        if GalleryImage.objects.filter(about_us=about_us, order=order).exists():
            continue
...
def unseed_gallery_images_retry(apps, schema_editor):
    AboutUs = apps.get_model("api", "AboutUs")
    GalleryImage = apps.get_model("api", "GalleryImage")
    about_us, _ = AboutUs.objects.get_or_create(id=1)
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(seed_gallery_images_retry, unseed_gallery_images_retry),
    ]
